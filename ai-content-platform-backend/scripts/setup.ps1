...
function Check-Port($name, $host, $port) {
    try {
        ...
        $tcp.Close()
        Write-Host "  + $name reachable on ${host}:${port}" -ForegroundColor Green
    } catch {
        Write-Host "  x $name NOT reachable on ${host}:${port}" -ForegroundColor Red
        $script:missing = $true
    }
}
...
Check-Port "Redis" "localhost" 6379
...
Write-Host "  copy .env.development .env"
